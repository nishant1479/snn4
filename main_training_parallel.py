...
from losses import build_temporal_loss
...
parser.add_argument('--loss',
                    default='tet',
                    choices=['tet', 'linear'],
                    help='Temporal loss type')
args = parser.parse_args()


def train(model, device, train_loader, criterion,temporal_loss, optimizer, epoch, args):
    running_loss = 0
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        if args.TET:
            loss, weights = temporal_loss(outputs, labels)
        else:
...
if __name__ == '__main__':
    ...
    temporal_loss = build_temporal_loss(
        loss_name=args.loss,
        criterion=criterion,
        means=args.means,
        lamb=args.lamb
    ...
    for epoch in range(args.epochs):
        ...
        loss, acc = train(parallel_model, device, train_loader, criterion,temporal_loss, optimizer, epoch, args)
